...
def list_model_files(model_paths: str, suffix: str = "") -> List[str]:
    try:
        ...
        model_files = [
            file
            for file in files
            if os.path.isfile(os.path.join(model_paths, file)) and file.endswith(suffix)
        ]
...
def load_data(
    train_data_path: dict, test_data_path: dict
...
def main(
    ...
    def load_and_evaluate_model(model_path: str) -> None:
        try:
            ...
            evaluate_model(
                model, x_train, y_train, x_test, y_test, config, seeds, device
            )
